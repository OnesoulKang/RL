...
class ActorCritic(nn.Module):
    ...
    def forward(self, state):
        ...
        pi = 2 * T.tanh(self.pi(pi1))
        sigma = F.softplus(self.sigma(pi1)) + 1e-3

        v = self.v(v1)

        return pi, sigma, v

    def calc_R(self, done, observation_):
        states = T.tensor(observation_, dtype=T.float)
    ...
    def calc_loss(self, done, observation_):
        states = T.tensor(self.states, dtype=T.float)
        actions = T.tensor(self.actions, dtype=T.float)

        returns = self.calc_R(done, observation_)
        returns = returns.detach()

        pi, sigma, values = self.forward(states)
        values = values.squeeze()
        critic_loss = (returns - values)**2

        dist = Normal(pi, sigma)
...
class Agent(mp.Process):
    ...
    def run(self):
        ...
        while self.episode_idx.value < N_GAMES:
            ...
            # while not done:
            for t_step in range(MAX_EP_STEP):
                action = self.local_actor_critic.choose_action(observation)
                observation_, reward, done, _ = self.env.step(
                    action.clip(-2, 2))

                if t_step == MAX_EP_STEP - 1:
                    done = True
