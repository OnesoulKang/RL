...
class PPO(nn.Module):
    ...
    def pi(self, x, softmax_dim = 0):
        ...
        # std = torch.tensor([0.5]) #F.softplus(self.fc_std(x))
...
def main():
    ...
    if not os.path.exists(file_path+"/save"):
    ...
    for n_epi in range(1000):
        ...
        if n_epi%print_interval==0 and n_epi!=0:
            ...
            if n_epi % (print_interval * 5)==0 and n_epi != 0:
                ...
                play_env = gym.make("BipedalWalker-v3", render_mode="human")
                s_, _ = play_env.reset()
                done_ = False
                score_ = 0
                step_ = 0
